...
int main(int argc, char const *argv[]) {
  ...
  for(int i = 0; i<N_steps-1;i++){
    ...
    force_half_step = force(position[i+1], momentum_half_step, gamma);
    momentum[i+1] = momentum_half_step + 0.5 * step_size * force_half_step;
    float t = (i+1)*step_size;
    forces[i+1] = force(position[i+1], momentum_half_step, gamma);
