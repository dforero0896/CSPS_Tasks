...
int main(int argc, char const *argv[]) {
  ...
  for(int i = 0; i<N_steps-1;i++){
    ...
    float momentum_predict = momentum[i] + step_size * forces[i];
    // Compute new force
    float force_predict = force(position_predict, momentum_predict, gamma);
    // Corrector step
    position[i+1] = position[i] + 0.5 * step_size * (momentum[i] + momentum_predict);
    momentum[i+1] = momentum[i] + 0.5 * step_size * (forces[i] + force_predict);
