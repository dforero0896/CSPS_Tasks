...
float force_derivative(float x){
  return -1;
}
...
int main(int argc, char const *argv[]) {
  ...
  for(int i = 0; i<N_steps-1;i++){
    position[i+1] = position[i] + step_size * momentum[i] + 0.5 * step_size * step_size * force(position[i]);
    momentum[i+1] = momentum[i] + step_size * force(position[i]) + 0.5 * step_size * step_size * force_derivative(position[i]) * momentum[i];
